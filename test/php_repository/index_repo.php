<?php
    ...
    if(isset($_POST["action"])){
        ...
        if($_POST["action"] == "usergetter"){
            ...
            $query .="AND IsActive = 1 and IsDeleted = 0 ORDER BY
            CASE 
              WHEN CurringBooth LIKE 'OSI%' THEN 1
              WHEN CurringBooth LIKE 'CAT%' THEN 2
              WHEN CurringBooth LIKE 'DC%' THEN 3
              WHEN CurringBooth LIKE 'Tunnel%' THEN 4
              ELSE 5 -- This handles any other values not specified
            END;";
            $fetch = odbc_exec($conn, $query);
        ...
        else if($_POST["action"]=="lineReject"){
            $processVal = isset($_POST['ProcessVal']) ? $_POST['ProcessVal'] : 0;
            $output ='';
            $query = "SELECT * ";
                $query .="FROM line_tbl ";
            ...
            if($counter !=0){
                ...
                $selectAll2 .= "order by SheetDetailID DESC ";
            ...
            $query .="WHERE IsActive = 1 and IsDeleted = 0 and UserWorkStationID = '".$_COOKIE['GPuserID']."' order by DateCreated DESC ";
